# FHIR data → AI insight in your pocket.

**An iOS app that connects to Epic via SMART on FHIR, pulls your clinical history, and asks an LLM to explain it in plain English.**
...
3. **Sends** a structured summary to an LLM (OpenAI `gpt-3.5-turbo` today) and gets back a plain-English insight.
4. **Displays** the AI summary alongside the raw data, so the patient sees both the structured truth and a human reading of it.

No PHI leaves the device until the patient explicitly taps "summarize."
...
Most patient-portal experiences still dump FHIR into tables nobody reads. This is a working proof that the whole loop — EHR OAuth → FHIR pull → LLM summary → native iOS UI — fits in a small app a solo developer can build in a weekend.
